...
# Import EWS data
df_ews = pd.read_csv('../output/df_ews_null.csv')

# Import DL prediction data
df_dl = pd.read_csv('../output/df_dl_null.csv')
...
def make_grid_figure(df_ews, df_dl, letter_label, title, transition=False):
    ...
    fig.add_trace(
        go.Scatter(x=df_ews['Beat number'],
                   y=df_ews['state'],
    ...
    fig.update_xaxes(title={'text':'Beat number',
                            'standoff':5},
                     ticks="outside",
                     showline=True,
                     linewidth=1,
    ...
    fig.update_yaxes(title={'text':'IBI (s)',
                            'standoff':5,
                            },
                     row=1,col=1)
...
list_img = []
filename = 'fig_ews_all_null_1.png'
...
# Create frame
dst = Image.new('RGB', 
                (6*ind_width, 2*ind_height), (255,255,255))
...
list_img = []
filename = 'fig_ews_all_null_2.png'
