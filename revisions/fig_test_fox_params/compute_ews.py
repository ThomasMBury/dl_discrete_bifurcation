...
for idx, alpha in enumerate(alpha_vals):
    ...
    for id_val in id_vals:
        ...
        s_forced = funs.simulate_model(A, B, C, D, tau, alpha, M0, D0, Tvals, sigma, sigma)['D']
        s_forced.plot()
        
        # Simulate a null trajectory up to bifurcation
        s_null = funs.simulate_model(A, B, C, D, tau, alpha, M0, D0, [Tstart]*niter, sigma, sigma)['D']
        s_null.plot()
        ...
        ts.apply_classifier(m1, tmin=0, tmax=(niter-1)*eval_pt, name='m1', verbose=0)
        ts.apply_classifier(m2, tmin=0, tmax=(niter-1)*eval_pt, name='m2', verbose=0)
