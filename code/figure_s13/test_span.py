...
eval_pts = np.arange(
    0.64, 1.01, 0.04
)  #  percentage of way through pre-transition time series
...
span_vals = [5, 10, 20, 40, 80]  # Lowess span (# beats)
...
for span in span_vals:
    print("Using a span of {}".format(span))
    ...
    for tsid in list_tsid:
        ...
        ts.detrend(
            method="Lowess", span=min(span, transition)
        )  # If span is bigger than available data take length of data)
    ...
    # Export data
    df_ktau_forced.to_csv(
        "output/df_ktau_pd_fixed_span_{}.csv".format(span), index=False
    )
    df_ktau_null.to_csv(
        "output/df_ktau_null_fixed_span_{}.csv".format(span), index=False
    )
