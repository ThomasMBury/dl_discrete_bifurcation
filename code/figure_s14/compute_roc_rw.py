...
rw_vals = np.arange(0.1, 0.7, 0.1)

for rw in rw_vals:
    print("Rolling window {}".format(rw))

    # Import df predictions
    df_ktau_forced = pd.read_csv("output/df_ktau_pd_fixed_rw_{:.1f}.csv".format(rw))
    df_ktau_null = pd.read_csv("output/df_ktau_null_fixed_rw_{:.1f}.csv".format(rw))
    ...
    # Export ROC data
    filepath = "output/df_roc_rw_{:.1f}.csv".format(rw)
    df_roc_full.to_csv(
        filepath,
        index=False,
    )
