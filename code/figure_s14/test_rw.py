...
# EWS parameters
rw_vals = np.arange(0.1, 0.7, 0.1)  # rolling window
...
for rw in rw_vals:
    print("Using a rolling window of {}".format(rw))
    # --------------
    ...
    df_ktau_forced.to_csv(
        "output/df_ktau_pd_fixed_rw_{:.1f}.csv".format(rw), index=False
    )
    df_ktau_null.to_csv(
        "output/df_ktau_null_fixed_rw_{:.1f}.csv".format(rw), index=False
    )
